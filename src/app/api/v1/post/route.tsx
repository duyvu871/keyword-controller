...
import {IPostData as TransformedData} from "@/types/postType";
...
interface OriginalData {
    ...
    inside: {
        ...
        description: string;
        detail: string;
        json_data: any[]; // Dữ liệu JSON không được xử lý trong ví dụ này
    };
}

const transformData = (originalData: OriginalData): TransformedData => {
    const transformedData: TransformedData = {
        title: originalData.title,
        previewImage: {
            url: originalData.inside.json_data[0]?.image?.url || '',
            size: {
                height: originalData.inside.json_data[0]?.image?.height || 0,
                width: originalData.inside.json_data[0]?.image?.width || 0,
            },
        },
        description: originalData.description,
        types: originalData.inside.json_data[0]?.about || [],
        redirect: originalData.link,
        timePost: originalData.inside.json_data[0]?.datePublished || "",
        author: originalData.inside.json_data[0]?.author.name || ""
    };
...
export async function GET(request: Request) {
    ...
    const file = await fs.readFile(process.cwd() + '/src/fake-data/vnExpress_Ukraine_post.json', 'utf8');
    const data: OriginalData[] = JSON.parse(file);
    // const path = "";
    ...
    const refactorData: TransformedData[] = data.map((item: OriginalData) => transformData(item));


    try {
        return new Response(JSON.stringify(refactorData), {
            status: 200,
            headers: {
